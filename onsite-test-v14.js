...
const businessDay={ready:false,loading:false,data:null,error:''};
...
function buttonSlot(b){return{waitTypeId:String(b?.dataset?.slot||''),waitTypeName:slotName(b)}}
function policyAllowed(b){return !!(businessDay.ready&&businessDay.data&&!businessDay.data.isClosed&&typeof P.isAllowedSlot==='function'&&P.isAllowedSlot(buttonSlot(b),businessDay.data.businessType))}
...
.policy-notice{margin:0 0 11px;padding:11px 12px;border-radius:13px;background:#edf5ff;color:#315a78;border:1px solid #b8d1e5;font-size:.7rem;line-height:1.55;font-weight:850}.policy-notice.ok{background:#e8f6ed;color:#245f3d;border-color:#abd4b9}.policy-notice.bad{background:#fff0ed;color:#85362e;border-color:#dda69e}
...
function ensurePolicyNotice(){
  let n=$('asbPolicyNotice');if(n)return n;
  n=document.createElement('div');n.id='asbPolicyNotice';n.className='policy-notice';
  const flow=$('receptionFlow'),slot=$('slotCard');
  if(flow&&slot)flow.insertBefore(n,slot);else flow?.prepend(n);
  return n
}
...
async function refreshBusinessDay(force=false){
  ...
  if(typeof P.getCurrentDay!=='function'){
    businessDay.ready=false;businessDay.error='営業日カレンダーの設定を読み込めません。';schedule();return;
  }
  businessDay.loading=true;businessDay.error='';schedule();
  try{
    const d=await P.getCurrentDay({force});
    if(!d||!d.businessType)throw new Error('営業区分を確認できませんでした');
    businessDay.data=d;businessDay.ready=true;businessDay.error='';
  }catch(e){businessDay.data=null;businessDay.ready=false;businessDay.error=String(e?.message||'営業日カレンダーを確認できませんでした')}
  finally{businessDay.loading=false;schedule()}
}
function dayMode(){
  if(businessDay.error)return{type:'営業日情報エラー',use:'受付を停止中'};
  if(!businessDay.ready||!businessDay.data)return{type:'営業区分を確認中',use:'読み込み中'};
  const type=String(businessDay.data.businessType||'');
  if(type==='平日')return{type:'平日',use:'時間制限なし'};
  if(type==='平日特定日')return{type:'平日特定日',use:'入場から3時間'};
  if(type==='土日祝日')return{type:'土日祝日',use:'入場から2時間30分'};
  if(type==='休館')return{type:'休館',use:'本日は休館'};
  return{type:'営業区分を確認中',use:'受付を停止中'};
}
...
function applySlotPolicy(){
  ...
  if(!buttons.length)setPolicyNotice('受付枠を読み込んでいます…');
  else if(!allowed)setPolicyNotice(`本日（${type}）に対応する受付枠が見つかりません。AirWAITの枠名・時間設定を確認してください。`,'bad');
  else setPolicyNotice(`本日は「${type}」です。対象の受付枠だけを表示しています。`,'ok');
}
...
function applyGate(){
  ...
  setHidden(box,false);
  if(businessDay.error){setHidden(flow,true);if(btn){btn.disabled=true;setText(btn,'営業日情報を確認できません')}if(st){setClass(st,'geo-state bad');setText(st,'営業日カレンダーを確認できないため受付を停止しています。')}return}
  if(!businessDay.ready||!businessDay.data){setHidden(flow,true);if(btn){btn.disabled=true;setText(btn,'営業日を確認中…')}if(st){setClass(st,'geo-state');setText(st,'本日の営業区分を確認しています。')}return}
  if(businessDay.data.isClosed){setHidden(flow,true);if(btn){btn.disabled=true;setText(btn,'本日は休館')}if(st){setClass(st,'geo-state bad');setText(st,'本日は休館日のため現地受付はできません。')}return}
...
function showArrivalGate(button){
  ...
  x.querySelector('.arrival-no').onclick=()=>{ackSlot='';x.remove();const buttons=[...list.querySelectorAll('.slot:not(:disabled):not([hidden])')],i=buttons.indexOf(button);buttons[i+1]?.scrollIntoView({behavior:'smooth',block:'center'})};
...
function decorateSlots(){
  ...
  const timed=[...list.querySelectorAll('button.slot[data-slot]:not([hidden])')].map(b=>({b,t:mins(extractTime(slotName(b)))})).filter(x=>x.t!==null).sort((a,b)=>a.t-b.t),now=nowMinutes();
  timed.forEach((x,i)=>{
...
function revealPolicyError(message){
  setPolicyNotice(message,'bad');
  const slot=$('slotCard'),people=$('peopleCard'),waiver=$('waiverCard');
  if(slot)slot.hidden=false;if(people)people.hidden=true;if(waiver)waiver.hidden=true;
  ensurePolicyNotice()?.scrollIntoView({behavior:'smooth',block:'center'});
}
function interceptSlot(e){
  const b=e.target.closest?.('#slotList button.slot[data-slot]');if(!b)return;
  if(!businessDay.ready||businessDay.error||!policyAllowed(b)){
    e.preventDefault();e.stopImmediatePropagation();revealPolicyError(businessDay.error?'営業日情報を確認できないため受付できません。':'本日の営業区分では、この受付枠は利用できません。');return;
  }
...
function interceptConfirm(e){
  ...
  if(G?.valid?.())return;
  e.preventDefault();e.stopImmediatePropagation();applyGate();G?.verify?.().then(applyGate)
}
...
function apply(){if(applying)return;applying=true;try{installStyle();renderSummary();ensurePolicyNotice();ensureGeo();applySlotPolicy();applyGate();decorateSlots();rewriteLinks();syncCallStatus()}finally{applying=false}}
...
setInterval(()=>refreshBusinessDay(true),60000);
window.addEventListener('pageshow',()=>{refreshBusinessDay(true);apply()});
document.addEventListener('visibilitychange',()=>{if(!document.hidden){refreshBusinessDay(true);apply()}});
setTimeout(apply,0);
setTimeout(()=>refreshBusinessDay(false),0);
})();
