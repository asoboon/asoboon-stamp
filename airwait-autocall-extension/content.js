...
const AUTO_KEY='asoboon_aw_auto_enabled_v17';
...
function buttonsIn(root){
  return [...root.querySelectorAll('button,[role="button"],a')].filter(visible);
}
...
function findDialogFor(receipt){
  ...
  return [...document.querySelectorAll(selectors)].filter(visible).find(el=>{
...
function ensureOverlay(){
  ...
  const host=document.createElement('div');host.id='asoboon-airwait-auto-v17';
