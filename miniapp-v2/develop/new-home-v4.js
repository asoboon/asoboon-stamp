...
function mount(){
  ...
  if(!current){
    const oldHome=root.querySelector('.home-shell');
    const viewHost=root.querySelector('main.view.active,main.view');
    if(oldHome)oldHome.outerHTML=shell();
    else if(viewHost)viewHost.innerHTML=shell();
    else return;
  }
...
window.addEventListener('asoboon:v2-liff-ready',()=>setTimeout(()=>{
  if(view()!=='home')return;
  lastHeroKey='';
  if(window.ASOBOON_HOME_STATUS_SNAPSHOT)latestStatus=window.ASOBOON_HOME_STATUS_SNAPSHOT;
  mount();
