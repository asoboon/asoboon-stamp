...
async function onStableUpdate({grid}={}){
  ...
  lastStableAt=Date.now();
  if(!ready||!CONFIG.ANIMATION_ENABLED||!CONFIG.IDLE_EVENTS_ENABLED||effectiveLevel()===0){diagnostics.skipped+=1;return{played:false,reason:'disabled'}}
  if(document.visibilityState==='hidden'||Date.now()<cooldownUntil||running||realFxBusy()){diagnostics.skipped+=1;return{played:false,reason:'busy'}}
  if(Math.random()>clamp(Number(CONFIG.IDLE_EVENT_CHANCE)||0,0,1)){diagnostics.none+=1;return{played:false,reason:'none'}}

  const tier=pickTier();
  const event=pickEvent(tier);
  if(!event){diagnostics.skipped+=1;return{played:false,reason:'no-event'}}
