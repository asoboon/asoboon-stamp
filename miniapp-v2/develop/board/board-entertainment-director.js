...
let ready=false,cooldownUntil=0,lastCharacterAt=0,lastPomponStoryAt=0,lastDuoStoryAt=0,lastRareAt=0;
let beatHistory=[],eventHistory=[],categoryBag=[],eventBags={},cycleNumber=0,cycleStartedAt=0,sequence=0;
const diagnostics={attempts:0,played:0,sourceFx:0,character:0,skipped:0,realInterrupts:0,suspends:0,cyclesCompleted:0,lastDecision:'none',lastEvent:null,history:[]};
...
function eligible(category,now,{ignoreCharacterForce=false}={}){
  ...
  if(!ignoreCharacterForce&&now-lastCharacterAt>=CONFIG.CHARACTER_FORCE_AFTER_MS&&category==='SOURCE_FX')return false;
  return true;
}
...
async function onStableUpdate({grid}={}){
  ...
  const result=category==='SOURCE_FX'?await SOURCEFX.play(eventId):await CHAR.play(eventId,{grid:grid||document.getElementById('queueGrid')});
  if(result?.played){
    diagnostics.played++;if(category==='SOURCE_FX')diagnostics.sourceFx++;else diagnostics.character++;
...
function getDiagnostics(){return{...diagnostics,history:diagnostics.history.map(x=>({...x})),ready,cooldownRemainingMs:Math.max(0,cooldownUntil-Date.now()),beatHistory:beatHistory.map(x=>({...x})),eventHistory:[...eventHistory],lastCharacterAt,lastPomponStoryAt,lastDuoStoryAt,lastRareAt,characterRate:diagnostics.played?diagnostics.character/diagnostics.played:0,cycleNumber,cycleStartedAt,remainingInCycle:categoryBag.length,bagSize:Object.values(BAG_TEMPLATE).reduce((a,b)=>a+b,0),legacyIdleInNormalRotation:false,scheduler:'shuffle-bag'}}
function resetForTest(){
  CHAR.cancel?.('test-reset');SOURCEFX.cancel?.('test-reset');ready=true;cooldownUntil=0;lastCharacterAt=Date.now();lastPomponStoryAt=0;lastDuoStoryAt=0;lastRareAt=0;
...
function simulateCycleForTest(seed=12345){
  ...
  const all=[...SOURCEFX.events,...CHAR.events.map(x=>x.id)];
