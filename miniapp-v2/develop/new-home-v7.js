...
function quick(kind,title,sub,target,panel=''){
  return `<button class="nh-quick nh-${kind}" type="button" data-nh-view="${target}"${panel?` data-nh-panel="${panel}"`:''}>
...
function play(kind,title,target){
  ...
  return `<button class="nh-fun nh-fun-${kind}" type="button" data-nh-view="${target}" ${enabled?'':'disabled'}>
...
function shell(){
  return `<main class="new-home" aria-label="ASOBooN HOME">
    ...
    <button id="nhToday" class="nh-today" type="button" data-nh-view="timeguide" aria-label="本日の営業・利用時間を確認">
    ...
    <section class="nh-guide" aria-labelledby="nhGuideTitle">
      ...
      <button class="nh-sub-link" type="button" data-nh-view="entry">
...
function heroMarkup(data={}){
  ...
  if(kind==='waiting'){
    ...
    return{
      ...
      html:`<div class="nh-statusline"><span class="nh-live-dot"></span><span>順番待ち</span></div>
        ...
        <button class="nh-primary nh-primary-dark" type="button" data-nh-view="callstatus">呼出状況を見る <span>›</span></button>`
    };
  }
  ...
  if(kind==='calling')return{
    ...
    html:`<div class="nh-eyebrow">呼出中</div><h1>入場できます！</h1>
      ...
      <button class="nh-primary nh-primary-green" type="button" data-nh-view="callstatus">受付番号・詳細を見る <span>›</span></button>`
  };
  ...
  if(kind==='guide')return{
    ...
    html:`<div class="nh-eyebrow">ご利用中</div><h1>ご利用中のご案内</h1>
      ...
      <button class="nh-primary nh-primary-dark" type="button" data-nh-view="entry">一時退場・再入場を見る <span>›</span></button>`
  };
  ...
  if(kind==='none'&&data.canceled){
    ...
    return{
      ...
      html:`<div class="nh-eyebrow">取消済み</div><h1>当日受付は取り消されています</h1>
        ...
        ${a.type==='open'?'<button class="nh-primary nh-primary-orange" type="button" data-nh-view="reception">当日受付をする <span>›</span></button>':''}`
    };
  }
  ...
  if(kind==='none'){
    ...
    if(a.type==='open')return{
      ...
      html:`<div class="nh-eyebrow">本日のご利用</div><h1>当日受付</h1>
        ...
        <button class="nh-primary nh-primary-orange" type="button" data-nh-view="reception">当日受付をする <span>›</span></button>`
    };
    return{
      cls:`nh-${a.type}`,
      html:`<div class="nh-eyebrow">${esc(a.eyebrow)}</div><h1>${esc(a.title)}</h1><p>${esc(a.message)}</p>
        <button class="nh-primary nh-primary-dark" type="button" data-nh-view="first">利用案内を見る <span>›</span></button>`
    };
  }
  ...
  return{
    ...
    html:`<div class="nh-eyebrow">確認中</div><h1>受付状況を確認中</h1>
      ...
      ${data.receipt?'<button class="nh-primary nh-primary-dark" type="button" data-nh-view="callstatus">呼出状況を確認する <span>›</span></button>':''}`
  };
}
...
function bindHostNavigation(host){
  if(host.dataset.nhNavBound==='1')return;
  host.dataset.nhNavBound='1';
  host.addEventListener('click',e=>{
    const target=e.target?.closest?.('[data-nh-view]');
    if(!target||target.disabled)return;
    e.preventDefault();
    const nav=window.ASOBOON_NEW_HOME_NAVIGATE;
    if(typeof nav!=='function')return;
    nav(String(target.dataset.nhView||''),{panel:String(target.dataset.nhPanel||'')});
  });
}
