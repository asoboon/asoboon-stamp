...
function routeUrl(view,extra={}){const u=endpointBase();u.searchParams.set('view',normalizeView(view));u.searchParams.set('mode',state.mode);u.searchParams.set('menu',state.mode);Object.entries(extra).forEach(([k,v])=>{if(v!==''&&v!=null)u.searchParams.set(k,String(v))});if(!isNewAppUrl(u.href))throw Error('DEVELOP_ROUTE_ESCAPE_BLOCKED');return u.href}
function go(view,{replace=false,...extra}={}){const u=routeUrl(view,extra);if(replace)history.replaceState({asoboonV2:true},'',u);else history.pushState({asoboonV2:true},'',u);readRoute();render();window.scrollTo({top:0,behavior:'smooth'});return true}
function setMode(mode){state.mode=normalizeMode(mode);const u=new URL(routeUrl(state.view));history.replaceState({asoboonV2:true},'',u.href);render()}
...
function bindAppEvents(){
  ...
  root.addEventListener('click',e=>{
    ...
    if(modeTarget&&!modeTarget.disabled){e.preventDefault();setMode(modeTarget.dataset.mode);return}
    ...
    if(target&&!target.disabled){
      ...
      const panel=isBack?'':(target.dataset.v7Panel||target.dataset.pv7Panel||'');
      if(Object.values(C.routes||{}).includes(String(view||''))){
        e.preventDefault();
        go(view,{replace:isBack,panel});
        return;
      }
    }
    ...
    if(href&&!isNewAppUrl(href)){e.preventDefault();console.error('DEVELOP_ROUTE_ESCAPE_BLOCKED',href)}
  },true);
}
...
async function initLiff(){
  ...
  try{
    ...
    await Promise.race([
      liff.init({liffId:E.liffId}),
      new Promise((_,reject)=>setTimeout(()=>reject(Error('LIFF_INIT_TIMEOUT')),6000))
    ]);
