...
function go(v){const u=endpoint();u.searchParams.set('view',v);u.searchParams.set('mode','before');location.href=u.href}
function readJSON(k){try{return JSON.parse(localStorage.getItem(k)||'null')}catch{return null}}
function yen(n){const x=Number(n);return Number.isFinite(x)?x.toLocaleString('ja-JP')+'円':'—'}
function dateLabel(s){const m=String(s||'').match(/^(\d{4})-(\d{2})-(\d{2})$/);return m?`${Number(m[2])}月${Number(m[3])}日`:String(s||'—')}
function setBrand(){const small=document.querySelector('.brand small');if(small)small.textContent='川口ハイウェイオアシス'}
...
function shell(eyebrow,title,lead,body){return `<section class="page-card pv7-page"><div class="pv7-head"><span class="pv7-eyebrow">${esc(eyebrow)}</span><h1>${esc(title)}</h1><p>${esc(lead)}</p></div><div class="pv7-body">${body}</div></section>`}
function pricePage(){return shell('料金','料金','料金はシンプル。ご家族の人数に合わせて受付画面で合計を自動計算します。',`
  <div class="pv7-list">
  ...
  <div class="pv7-callout orange"><strong>0〜5か月のお子さまについて</strong><br>お子さま1人目にあたる場合は900円。2人目以降にあたる場合は追加料金なしです。</div>
...
function entryPage(){return shell('入場・退場','入場・退場','いま何をすればいいか迷わないよう、受付から退場までを一本道でまとめています。',`
  <div class="pv7-roadflow">
    <div class="pv7-roadstep"><b>1</b><strong>受付を完了</strong><small>受付番号が発行されたら、その番号があなたの受付です。</small></div>
...
function rulesPage(){return shell('館内ルール','たのしく遊ぶための約束','禁止事項を並べるのではなく、みんなが安心して遊ぶための大切なことをまとめています。',`
  ...
  <div class="pv7-promise"><b>3</b><div><strong>遊具は遊び方を考えながら</strong><small>子どもの挑戦を大切にしながら、重大な事故につながる危険は避けて遊びましょう。</small></div></div>
  <div class="pv7-promise"><b>4</b><div><strong>困ったときはスタッフへ</strong><small>けが・迷子・体調不良・遊具の不具合などは近くのスタッフへお知らせください。</small></div></div>
  <div class="pv7-callout green">スタッフから安全のためにお声がけする場合があります。館内での案内を優先してください。</div>`)}
function infoPage(){return shell('その他のご案内','その他のご案内','営業時間や施設の基本情報をまとめています。',`
  <div class="pv7-list">
    <div class="pv7-band" style="--band:var(--pv7-road)"><div><strong>営業時間</strong><small>平日 10:00〜17:00</small></div><b>平日</b></div>
    <div class="pv7-band" style="--band:var(--pv7-orange)"><div><strong>土日祝</strong><small>10:00〜18:00</small></div><b>18:00</b></div>
...
function detailPage(){const r=readJSON(RES_KEY);if(!r?.receiptNo)return shell('受付詳細','本日の受付はありません','受付が完了すると、ここに受付番号や利用回が表示されます。',`<div class="pv7-callout">HOMEから受付を行うと、このページに受付内容を保存します。</div><button class="pv7-primary" type="button" data-pv7-view="reception">受付する</button>`);
  const kids=Number(r.paidChildren||0)+Number(r.infants||0);return shell('受付詳細','受付詳細','現在の端末に保存されている本日の受付内容です。',`
  <div class="pv7-list">
...
function staticHtml(v){if(v==='price')return pricePage();if(v==='first')return firstPage();if(v==='entry')return entryPage();if(v==='rules')return rulesPage();if(v==='info'||v==='parking')return infoPage();if(v==='reservationdetail')return detailPage();return''}
function replaceStatic(v){const html=staticHtml(v);if(!html)return false;const main=root.querySelector('main.view');if(!main)return false;if(main.dataset.pv7View===v)return true;main.innerHTML=html;main.dataset.pv7View=v;return true}
function patchCallstatus(){const page=root.querySelector('.cs-page');if(!page)return;document.body.classList.add('v7-page-active','v7-callstatus-active');const head=page.querySelector('.page-head');if(head){const s=head.querySelector('small'),h=head.querySelector('h2');if(s)s.textContent='CALL STATUS';if(h)h.textContent='呼出状況'}const refresh=page.querySelector('#csRefresh');if(refresh&&!page.querySelector('.pv7-detail-link'))refresh.insertAdjacentHTML('afterend','<button class="pv7-text-btn pv7-detail-link" type="button" data-pv7-view="reservationdetail">受付詳細を見る</button>')}
function patchTimeguide(){const page=root.querySelector('.tg-page');if(!page)return;document.body.classList.add('v7-page-active','v7-timeguide-active');const head=page.querySelector('.page-head');if(head){const s=head.querySelector('small'),h=head.querySelector('h2');if(s)s.textContent='PLAY TIME';if(h)h.textContent='利用時間の目安'}const body=page.querySelector('.tg-body');if(body&&!body.querySelector('.pv7-time-warning'))body.insertAdjacentHTML('afterbegin','<div class="pv7-time-warning">入場時刻を自分で選んで計算する目安です。実際の入場時刻を自動判定しているわけではありません。</div>')}
function rewireHome(){const home=root.querySelector('.v7-home');if(!home)return;home.querySelectorAll('.v7-guide-row').forEach(btn=>{const t=String(btn.querySelector('strong')?.childNodes?.[0]?.textContent||btn.querySelector('strong')?.textContent||'').trim();if(t.startsWith('料金'))btn.dataset.v7View='price'});home.querySelectorAll('.v7-secondary').forEach(btn=>{if(/受付詳細/.test(btn.textContent||''))btn.dataset.v7View='reservationdetail'});const more=home.querySelector('.v7-more');if(more)more.dataset.v7View='info'}
function bindOwn(){root.querySelectorAll('[data-pv7-view]').forEach(el=>{if(el.dataset.pv7Bound==='1')return;el.dataset.pv7Bound='1';el.addEventListener('click',()=>go(el.dataset.pv7View))})}
function apply(){queued=false;const v=view();clearClasses();setBrand();if(v==='home'){rewireHome();return}if(v==='reception')return;if(v==='callstatus'){patchCallstatus();bindOwn();return}if(v==='timeguide'){patchTimeguide();bindOwn();return}if(['price','first','entry','rules','info','parking','reservationdetail'].includes(v)){document.body.classList.add('v7-page-active','v7-content-active');replaceStatic(v);bindOwn();return}}
