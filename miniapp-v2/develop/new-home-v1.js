...
function availability(){
  ...
  if(t==='平日'){
    ...
    return{type:'onsite',eyebrow:'現地受付',title:'本日は現地受付です',message:'通常平日はLINE当日受付を行っていません。ASOBooN入口で受付してください。'};
  }
  if(now.minutes<420)return{type:'before',eyebrow:'受付開始前',title:'LINE当日受付は7:00から',message:'オンライン受付対象日は7:00から受付できます。'};
  return{type:'open'};
}
...
function quick(kind,title,sub,target,panel=''){
  return `<button class="nh-quick nh-${kind}" type="button" data-v7-view="${target}"${panel?` data-v7-panel="${panel}"`:''}>
...
function play(kind,title,target){
  ...
  return `<button class="nh-fun nh-fun-${kind}" type="button" data-v7-view="${target}" ${enabled?'':'disabled'}>
    <span class="nh-fun-icon">${icon(kind)}</span>
    <strong>${title}</strong>
    ${enabled?'':'<small>準備中</small>'}
  </button>`;
}
...
function shell(){
  return `<main class="new-home" aria-label="ASOBooN HOME">
    ...
    <button id="nhToday" class="nh-today" type="button" data-v7-view="timeguide" aria-label="本日の営業・利用時間を確認">
    ...
    <section class="nh-guide" aria-labelledby="nhGuideTitle">
      <header class="nh-heading">
        <span>GUIDE</span>
        <h2 id="nhGuideTitle">すぐ見つかる案内</h2>
      </header>
      <div class="nh-quick-grid">
        ${quick('first','初めての方','受付〜入場まで','first')}
        ${quick('price','料金','料金・人数ルール','first','price')}
        ${quick('parking','駐車場・アクセス','一般道・高速道路','parking')}
        ${quick('rules','館内ルール','安全に遊ぶために','rules')}
      </div>
      <button class="nh-sub-link" type="button" data-v7-view="entry">
...
function heroMarkup(data={}){
  ...
  if(kind==='waiting'){
    ...
    return{
      cls:'nh-waiting',
      html:`<div class="nh-statusline"><span class="nh-live-dot"></span><span>呼出待ち</span></div>
        ...
        <button class="nh-primary nh-primary-dark" type="button" data-v7-view="callstatus">呼出状況を見る <span>›</span></button>`
    };
  }

  if(kind==='calling')return{
    cls:'nh-calling',
    html:`<div class="nh-eyebrow">NOW CALLING</div><h1>入場できます！</h1>
      <p><strong>呼出後30分以内</strong>にASOBooN入口へお越しください。</p>
      <button class="nh-primary nh-primary-green" type="button" data-v7-view="callstatus">受付番号・詳細を見る <span>›</span></button>`
  };

  if(kind==='guide')return{
    cls:'nh-guide-state',
    html:`<div class="nh-eyebrow">INFORMATION</div><h1>スタッフのご案内をご確認ください</h1>
      <p>一時退場・再入場のルールはこちらから確認できます。</p>
      <button class="nh-primary nh-primary-dark" type="button" data-v7-view="entry">一時退場・再入場を見る <span>›</span></button>`
  };
  ...
  if(kind==='none'&&data.canceled){
    ...
    return{
      cls:'nh-canceled',
      html:`<div class="nh-eyebrow">CANCELED</div><h1>受付は取消になっています</h1>
        <p>${a.type==='open'?'新しく順番を取る場合は、当日受付からお進みください。':esc(a.message)}</p>
        ${a.type==='open'?'<button class="nh-primary nh-primary-orange" type="button" data-v7-view="reception">当日受付をする <span>›</span></button>':''}`
    };
  }
  ...
  if(kind==='none'){
    ...
    if(a.type==='open')return{
      cls:'nh-open',
      html:`<div class="nh-eyebrow">TODAY</div><h1>本日の当日受付</h1>
        <p>利用する回と人数を選んで、LINEで順番を取ります。</p>
        <button class="nh-primary nh-primary-orange" type="button" data-v7-view="reception">当日受付をする <span>›</span></button>`
    };
    return{
      cls:`nh-${a.type}`,
      html:`<div class="nh-eyebrow">${esc(a.eyebrow)}</div><h1>${esc(a.title)}</h1><p>${esc(a.message)}</p>
        <button class="nh-primary nh-primary-dark" type="button" data-v7-view="first">利用案内を見る <span>›</span></button>`
    };
  }

  return{
    cls:'nh-sync',
    html:`<div class="nh-eyebrow">SYNC</div><h1>受付状況を確認しています</h1>
      <p>${esc(data.message||'最新の受付状況を確認しています。')}</p>
      ${data.receipt?'<button class="nh-primary nh-primary-dark" type="button" data-v7-view="callstatus">呼出状況を確認する <span>›</span></button>':''}`
  };
}
...
function mount(){
  ...
  const current=root.querySelector('.new-home');
  if(!current){
    const old=root.querySelector('.home-shell')||root.firstElementChild;
    if(old)old.outerHTML=shell(); else root.innerHTML=shell();
  }
