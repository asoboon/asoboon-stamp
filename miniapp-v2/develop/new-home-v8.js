...
function routeHref(target,panel=''){
  const u=new URL(E.endpoint||location.href,location.href);
  u.search='';
  u.hash='';
...
function quick(kind,title,sub,target,panel=''){
  return `<a class="nh-quick nh-${kind}" href="${routeHref(target,panel)}" data-nh-view="${target}"${panel?` data-nh-panel="${panel}"`:''}>
...
function play(kind,title,target){
  ...
  return `<a class="nh-fun nh-fun-${kind}" href="${routeHref(target)}" data-nh-view="${target}">
    <span class="nh-fun-icon">${icon(kind)}</span>
    <strong>${title}</strong>
  </a>`;
}
...
function shell(){
  return `<main class="new-home" aria-label="ASOBooN HOME">
    ...
    <a id="nhToday" class="nh-today" href="${routeHref('timeguide')}" data-nh-view="timeguide" aria-label="本日の営業・利用時間を確認">
    ...
    <section class="nh-guide" aria-labelledby="nhGuideTitle">
      ...
      <a class="nh-sub-link" href="${routeHref('entry')}" data-nh-view="entry">
...
function heroMarkup(data={}){
  ...
  if(kind==='waiting'){
    ...
    return{
      ...
      html:`<div class="nh-statusline"><span class="nh-live-dot"></span><span>順番待ち</span></div>
        ...
        <a class="nh-primary nh-primary-dark" href="${routeHref('callstatus')}" data-nh-view="callstatus">呼出状況を見る <span>›</span></a>`
    };
  }
  ...
  if(kind==='calling')return{
    ...
    html:`<div class="nh-eyebrow">呼出中</div><h1>入場できます！</h1>
      ...
      <a class="nh-primary nh-primary-green" href="${routeHref('callstatus')}" data-nh-view="callstatus">受付番号・詳細を見る <span>›</span></a>`
  };
  ...
  if(kind==='guide')return{
    ...
    html:`<div class="nh-eyebrow">ご利用中</div><h1>ご利用中のご案内</h1>
      ...
      <a class="nh-primary nh-primary-dark" href="${routeHref('entry')}" data-nh-view="entry">一時退場・再入場を見る <span>›</span></a>`
  };
  ...
  if(kind==='none'){
    ...
    if(a.type==='open')return{
      ...
      html:`<div class="nh-eyebrow">本日のご利用</div><h1>当日受付</h1>
        ...
        <a class="nh-primary nh-primary-orange" href="${routeHref('reception')}" data-nh-view="reception">当日受付をする <span>›</span></a>`
    };
    return{
      cls:`nh-${a.type}`,
      html:`<div class="nh-eyebrow">${esc(a.eyebrow)}</div><h1>${esc(a.title)}</h1><p>${esc(a.message)}</p>
        <a class="nh-primary nh-primary-dark" href="${routeHref('first')}" data-nh-view="first">利用案内を見る <span>›</span></a>`
    };
  }
...
function bindHostNavigation(host){
  ...
  host.addEventListener('click',e=>{
    ...
    try{
      const handled=nav(String(target.dataset.nhView||''),{panel:String(target.dataset.nhPanel||'')});
      if(handled!==false)e.preventDefault();
    }catch(err){
      console.error('NEW_HOME_NAVIGATION_FAILED',err);
    }
  });
}
