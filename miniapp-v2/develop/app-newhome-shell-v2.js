...
function go(view,{replace=false,...extra}={}){const v=normalizeView(view);if(state.booting&&state.inClient&&!state.liffReady){pendingNavigation={view:v,replace,extra};state.view=v;render();window.scrollTo({top:0,behavior:'smooth'});return true}const u=routeUrl(v,extra);if(replace)history.replaceState({asoboonV2:true},'',u);else history.pushState({asoboonV2:true},'',u);readRoute();render();window.scrollTo({top:0,behavior:'smooth'});return true}
...
function actionCard(view,kind,title,desc,cls=''){const enabled=feature(view);return `<button class="flow-card ${cls}" data-app-view="${view}" type="button"><span class="mini">${esc(kind)}</span><strong>${esc(title)}</strong><p>${esc(desc)}</p><span class="arrow">›</span>${enabled?'':'<span class="soon">準備中</span>'}</button>`}
function funCard(view,emoji,title,sub){const enabled=feature(view);return `<button class="fun-card" data-app-view="${view}" type="button"><span class="emoji">${emoji}</span><strong>${esc(title)}</strong><small>${esc(sub)}</small>${enabled?'':'<span class="soon">統合中</span>'}</button>`}
function home(){return `<section class="home-shell new-home-shell"><div id="newHomeHost" aria-live="polite"></div></section>`}
...
function shell(){const homeView=state.view==='home';const lineText=state.booting?'LINE接続中':state.liffReady?(state.inClient?'LINE内':'ブラウザプレビュー'):(state.bootError?'LIFF確認エラー':'LIFF未接続');const dot=state.liffReady?(state.inClient?'ok':'warn'):(state.bootError?'warn':'');return `<div class="devbar"><span><i class="dot ${dot}"></i><b>🧪 NEW HOME · ${esc(E.environmentLabel||'ENV')}</b></span><span>${esc(lineText)} · v${esc(C.version||'')}</span></div><header class="topbar"><button class="iconbtn ${homeView?'ghost':''}" id="appBackBtn" type="button" aria-label="新HOMEへ戻る">‹</button><div class="brand"><strong>ASOBooN</strong><small>NEW OFFICIAL LINE MINI APP</small></div><button class="iconbtn ${homeView?'ghost':''}" data-app-view="home" type="button" aria-label="新HOME">⌂</button></header><main class="view active">${pageFor(state.view)}</main><footer class="footer"><b>ASOBooN NEW HOME / MINI APP v2</b><br>${esc(E.environmentLabel||'')} · isolated routing · legacy HOME blocked</footer>`}
...
function bind(){root.querySelectorAll('[data-app-view]').forEach(el=>el.addEventListener('click',()=>go(el.dataset.appView)));root.querySelectorAll('[data-mode]').forEach(el=>el.addEventListener('click',()=>setMode(el.dataset.mode)));const back=document.getElementById('appBackBtn');if(back&&!back.classList.contains('ghost'))back.addEventListener('click',()=>go('home',{replace:true}));root.addEventListener('click',e=>{const a=e.target.closest?.('a[href]');if(!a)return;if(a.hasAttribute('data-external'))return;const href=a.href||a.getAttribute('href');if(href&&!isNewAppUrl(href)){e.preventDefault();e.stopPropagation();console.error('NEW_HOME_ROUTE_ESCAPE_BLOCKED',href)}},true);bindModule()}
function render(){cleanupModules();document.title='ASOBooN 新HOME｜LINEミニアプリ';root.innerHTML=shell();bind();window.dispatchEvent(new CustomEvent('asoboon:v2-rendered',{detail:{view:state.view,mode:state.mode}}))}
...
async function initLiff(){
  ...
  try{
    ...
    if(pendingNavigation){const p=pendingNavigation;pendingNavigation=null;go(p.view,{replace:p.replace,...p.extra})}
    if(liff.isLoggedIn()){
...
window.ASOBOON_NEW_HOME_NAVIGATE=go;
