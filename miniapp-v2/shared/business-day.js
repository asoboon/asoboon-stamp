...
async function request(date){try{return await fetchCors(date)}catch(first){try{return await fetchJsonp(date)}catch(second){const e=Error('営業カレンダーに接続できませんでした。');e.cause={first,second};throw e}}}
async function getByDate(value,{force=false}={}){const date=normalizeDate(value);if(!date)throw Error('日付形式が不正です。');if(!force){const cached=readCache(date);if(cached)return cached;if(inflight.has(date))return inflight.get(date)}const job=(async()=>{const payload=await request(date),result=normalize(payload,date);writeCache(date,payload);return result})();inflight.set(date,job);try{return await job}finally{inflight.delete(date)}}
