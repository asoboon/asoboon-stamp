...
(()=>{'use strict';
const VERSION='1.2.0-resilient-cache';
...
function readCache(date,maxAge=CACHE_MS){try{const x=JSON.parse(localStorage.getItem(CACHE_KEY)||'{}'),v=x?.[date];if(!v||Date.now()-Number(v.savedAt||0)>maxAge)return null;return normalize(v.payload,date,maxAge>CACHE_MS?'cache-stale':'cache')}catch{return null}}
...
async function getByDate(value,{force=false}={}){const date=normalizeDate(value);if(!date)throw Error('日付形式が不正です。');const stale=readCache(date,STALE_CACHE_MS);if(!force){const cached=readCache(date);if(cached)return cached;if(inflight.has(date))return inflight.get(date)}const job=(async()=>{try{const payload=await request(date),result=normalize(payload,date,String(payload?.source||'api'));writeCache(date,payload);return result}catch(e){if(stale)return stale;throw e}})();inflight.set(date,job);try{return await job}finally{inflight.delete(date)}}
