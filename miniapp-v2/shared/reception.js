...
function usageAllowed(actual,mode,{developTest=false,weekdayAll=false}={}){const u=String(actual?.usageDispType||'');if(developTest)return !u||['01','02','KeyALL','KeySTORE_RECEPTION_ONLY'].includes(u);if(weekdayAll)return !u||['01','02','03','KeyALL','KeySTORE_RECEPTION_ONLY','KeyONLINE_RECEPTION_ONLY'].includes(u);if(!u||u==='01'||u==='KeyALL')return true;if(mode==='web')return u==='03'||u==='KeyONLINE_RECEPTION_ONLY';return u==='02'||u==='KeySTORE_RECEPTION_ONLY'}
function buildSlots(waitTypes){const test=developTestRule(),configured=DEVELOP_TEST_ONLY?(test?[test]:[]):[...(S.day&&typeof R.slotsFor==='function'?R.slotsFor(S.day.businessType,S.mode):[])],weekdayAll=!DEVELOP_TEST_ONLY&&S.mode==='web'&&String(S.day?.businessType||'')==='平日';return configured.map(rule=>{const actual=Array.isArray(waitTypes)?waitTypes.find(x=>String(x.waitTypeId||'')===String(rule.waitTypeId)):null;if(!actual)return null;if(rule.developTest){if(!usageAllowed(actual,S.mode,{developTest:true}))return null;return{...rule,actual}}if(actual.dispFlg===false||!usageAllowed(actual,S.mode,{weekdayAll}))return null;return{...rule,actual}}).filter(Boolean)}
