...
function pageFor(view){if(view==='home')return home();if(!feature(view)){const meta={reception:['当日受付','🎫'],callstatus:['呼出状況','🔔'],stamp:['スタンプラリー','🔩'],omikuji:['おみくじ','🎯'],game:['ミニゲーム','🏎️']}[view]||[view,'🛠️'];return disabledPage(view,meta[0],meta[1])}if(view==='reception')return receptionPage();if(view==='callstatus')return callstatusPage();if(view==='first')return firstPage();if(view==='entry')return entryPage();if(view==='rules')return rulesPage();if(view==='timeguide')return timePage();if(view==='parking')return parkingPage();return disabledPage(view,view,'🛠️')}
...
function cleanupModules(){try{callstatusModule()?.unmount?.()}catch(e){console.warn('CALLSTATUS_UNMOUNT_FAILED',e)}}
function bindModule(){
