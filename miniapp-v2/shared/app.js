...
function receptionPage(){const mod=window.ASOBOON_V2_RECEPTION;return mod&&typeof mod.render==='function'?mod.render():disabledPage('reception','当日受付','🎫')}
...
function timePage(){return `<section class="page-card"><div class="page-head green"><small>PLAY TIME</small><h2>何時まであそべる？</h2></div><div class="page-body"><h3>利用時間は営業区分で変わります</h3><p>新版では営業カレンダーを正本にし、曜日から推測しません。入場時刻を登録すると、その日の利用時間・閉館時刻を使って終了目安を表示します。</p><div class="timebox"><small>新HOME段階</small><strong>営業日エンジン接続済み</strong><p>受付画面と同じ営業カレンダーを利用します。入場時刻計算は次の統合工程で接続します。</p></div></div></section>`}
...
function pageFor(view){if(view==='home')return home();if(!feature(view)){const meta={reception:['当日受付','🎫'],callstatus:['呼出状況','🔔'],stamp:['スタンプラリー','🔩'],omikuji:['おみくじ','🎯'],game:['ミニゲーム','🏎️']}[view]||[view,'🛠️'];return disabledPage(view,meta[0],meta[1])}if(view==='reception')return receptionPage();if(view==='first')return firstPage();if(view==='entry')return entryPage();if(view==='rules')return rulesPage();if(view==='timeguide')return timePage();if(view==='parking')return parkingPage();return disabledPage(view,view,'🛠️')}
