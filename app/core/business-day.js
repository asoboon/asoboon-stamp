...
(()=>{
  ...
  function pruneCache(store){
    ...
    Object.keys(store).forEach(date=>{
      if(Number(store[date]?.savedAt||0)<cutoff)delete store[date];
    });
    return store;
  }
  ...
  function readCache(date){
    ...
    const raw=store[date];
    if(!raw||Date.now()-Number(raw.savedAt||0)>=CACHE_MS)return null;
    try{return normalizePayload(raw.payload,date,'cache')}
    catch{return null}
  }

  function writeCache(date,payload){
    const store=pruneCache(readCacheStore());
    store[date]={savedAt:Date.now(),payload};
  ...
  function normalizePayload(payload,requestedDate,source='api'){
    ...
    if(payload.ok!==true)throw new Error(String(payload.message||payload.code||'営業日を取得できませんでした。'));
    ...
    return Object.freeze({
      ...
      isClosed:rule.isClosed,durationMinutes:rule.durationMinutes,
      durationLabel:rule.durationLabel,closingTime:rule.closingTime,
      note:String(payload.note||''),row:Number(payload.row||0)||null
    });
  }

  async function fetchJson(date){
    const u=new URL(API_URL);
    u.searchParams.set('action','current');u.searchParams.set('date',date);u.searchParams.set('_',String(Date.now()));
  ...
  function fetchJsonp(date){
    return new Promise((resolve,reject)=>{
      ...
      const u=new URL(API_URL);
      u.searchParams.set('action','current');u.searchParams.set('date',date);u.searchParams.set('callback',cb);u.searchParams.set('_',String(Date.now()));
  ...
  async function request(date){
    let firstError=null;
    try{return await fetchJson(date)}catch(e){firstError=e}
    try{return await fetchJsonp(date)}catch(e){
      const err=new Error('営業日カレンダーに接続できませんでした。');
      err.cause={fetch:firstError,jsonp:e};throw err;
    }
  }
  ...
  async function getByDate(value,{force=false}={}){
    ...
    if(!force){
      const cached=readCache(date);if(cached)return cached;
    ...
    const job=(async()=>{
      ...
      writeCache(date,payload);return result;
    })();
    inflight.set(date,job);
    try{return await job}finally{inflight.delete(date)}
  }

  function getCurrent(options={}){return getByDate(jstDate(),options)}
  function getRule(businessType){return RULES[String(businessType||'').trim()]||null}

  window.ASOBOON_BUSINESS_DAY=Object.freeze({
    version:VERSION,apiUrl:API_URL,timeZone:TIME_ZONE,cacheMs:CACHE_MS,
    rules:RULES,normalizeDate,getRule,getByDate,getCurrent,clearCache
  });
})();
